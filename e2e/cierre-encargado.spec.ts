...
import { STAFFS, loginAsAdmin } from "./helpers";
...
test.describe("Encargados de cierre de caja", () => {
  ...
  test("un empleado común no ve ni accede a cerrar caja", async ({ page }) => {
    ...
    await page.goto("/terminal/cierre");
    await expect(page.getByText("Sin permiso")).toBeVisible();
  });

  test("un empleado encargado puede cerrar la caja de su sucursal", async ({ page }) => {
    // El admin marca a Nico como encargado.
    await loginAsAdmin(page);
    await page.goto("/staff");
    await page.getByRole("button", { name: new RegExp(STAFFS.nico.name) }).click();
    ...
    await expect(page.getByText("Vendedor actualizado.").first()).toBeVisible();
