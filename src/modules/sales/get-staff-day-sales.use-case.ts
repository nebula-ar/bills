import { findStaffSalesInRange } from "./sale.repository";
...
export async function getStaffDaySales(input: { branchId: string; staffId: string; now: Date }) {
  const { start, end } = argentinaDayRange(input.now);
  return findStaffSalesInRange({ branchId: input.branchId, staffId: input.staffId, start, end });
}
