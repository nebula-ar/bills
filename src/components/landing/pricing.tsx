...
export function PricingSection() {
  return (
    <section id="precios" className="border-t border-slate-200 bg-[var(--background)] py-24 text-slate-950 lg:py-32">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        ...
        <div className="mt-14 grid items-start gap-4 lg:grid-cols-3">
          {landingPlans.map((plan) => (
            <article key={plan.name} className={`relative flex h-full flex-col rounded-3xl border p-7 ${plan.highlighted ? "border-slate-950 bg-slate-950 text-white shadow-[0_24px_60px_-22px_rgba(17,19,21,0.7)] lg:-translate-y-3" : "border-slate-300 bg-[var(--card)]"}`}>
