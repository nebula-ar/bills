...
function DashboardPreview({ example }: { example: RubroExample }) {
  return (
    <div className="relative w-full max-w-[590px]">
      ...
      <div className="absolute -bottom-4 -left-3 z-10 rotate-[-6deg] rounded-2xl border border-slate-950 bg-[var(--accent-brand)] px-4 py-3 text-xs font-black text-slate-950 shadow-[7px_8px_0_#111315] sm:-left-8">
      ...
      <div className="rotate-[1.5deg] rounded-[28px] border border-slate-950/10 bg-[var(--card)] p-3 shadow-[0_30px_80px_-20px_rgba(17,19,21,0.32)] sm:p-4">
        <div className="flex items-center justify-between border-b border-slate-200/80 px-2 pb-3">
          <div className="flex items-center gap-2 text-xs font-black tracking-tight text-slate-950">
            <span className="grid h-6 w-6 place-items-center rounded-lg bg-slate-950 text-[11px] font-black text-[var(--accent-brand)]">B</span>
        ...
        <div className="grid min-h-[395px] grid-cols-[88px_1fr] sm:grid-cols-[132px_1fr]">
          <aside className="border-r border-slate-200/80 px-1 py-5 sm:px-2">
            <div className="mb-6 flex items-center gap-2 px-1 text-xs font-black text-slate-950">
              <span className="grid h-6 w-6 place-items-center rounded-lg bg-slate-950 text-[11px] text-[var(--accent-brand)]">B</span>
              <span className="hidden sm:inline">Bills</span>
            </div>
            <div className="space-y-1 text-[10px] font-bold sm:text-[11px]">
              <div className="rounded-lg bg-[#eef1ff] px-2 py-2 text-[var(--primary)]">Resumen</div>
          ...
          <div className="min-w-0 px-3 py-5 sm:px-5">
            ...
            <div className="mt-5 flex items-end justify-between rounded-2xl bg-slate-950 p-4 text-white">
              <div>
                ...
                <p className="mt-1 text-[9px] font-extrabold text-[var(--accent-brand)]">{example.metricChange}</p>
              </div>
              <div className="flex h-10 items-end gap-1" aria-hidden="true">
                {[18, 28, 22, 35, 25, 39].map((height, index) => (
                  <span key={index} className="w-1.5 rounded-full bg-[var(--accent-brand)]" style={{ height }} />
                ))}
              </div>
            </div>
            ...
            <div className="mt-5 border-t border-slate-200/80 pt-3">
              <div className="mb-1 flex items-center justify-between">
                ...
                <span className="text-[10px] font-extrabold text-[var(--primary)]">Ver todo</span>
              </div>
              {example.activity.map((item) => (
                <div key={item.title} className="flex items-center justify-between gap-2 border-b border-slate-100 py-2 last:border-0">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-[#eef1ff] text-[10px] font-black text-[var(--primary)]">{item.initials}</span>
...
export function HeroSection() {
  ...
  return (
    <section id="hero" className="relative overflow-hidden bg-[var(--background)] text-slate-950">
      <div className="mx-auto grid min-h-[760px] max-w-7xl items-center gap-12 px-5 pb-20 pt-32 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:px-12 lg:pb-28 lg:pt-44">
        <div>
          ...
          <h1 className="mt-6 max-w-2xl text-[clamp(3.25rem,7vw,5.9rem)] font-black leading-[0.94] tracking-[-0.08em] text-slate-950">
            {titleStart}, <span className="text-[var(--primary)]">{titleAccent}</span>
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/register" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white transition hover:-translate-y-0.5 hover:bg-[var(--primary)]">
              ...
              {rubroExamples.map((example) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex min-h-11 items-center gap-2 rounded-full border px-3.5 text-xs font-extrabold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2 ${active ? "border-[var(--primary)] bg-[var(--primary)] text-white" : "border-slate-300 text-slate-600 hover:border-[var(--primary)] hover:text-[var(--primary)]"}`}
                  >
      ...
      <div className="border-y border-slate-800 bg-slate-950 px-5 py-4 text-white sm:px-8 lg:px-12">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <p className="text-xs text-slate-400">Un sistema para el negocio que ya tenés — <strong className="text-[var(--accent-brand)]">y el que querés construir.</strong></p>
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-bold text-slate-300">
            <span><CircleDollarSign className="mr-1 inline h-3.5 w-3.5 text-[var(--accent-brand)]" />Ventas</span>
            <span><WalletCards className="mr-1 inline h-3.5 w-3.5 text-[var(--accent-brand)]" />Caja</span>
            <span><Box className="mr-1 inline h-3.5 w-3.5 text-[var(--accent-brand)]" />Stock</span>
            <span><UsersRound className="mr-1 inline h-3.5 w-3.5 text-[var(--accent-brand)]" />Clientes</span>
          </div>
        </div>
      </div>
    </section>
