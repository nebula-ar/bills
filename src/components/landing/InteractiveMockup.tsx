  ...
  return (
    <div className="flex-1 p-6 flex flex-col gap-5 relative h-full bg-slate-50 min-h-[400px]">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="h-10 w-10 bg-primary/15 border border-primary/20 rounded-full flex items-center justify-center shadow-sm">
          ...
          <div
            ...
            className={`p-4 rounded-2xl border transition-all cursor-pointer flex flex-col gap-3 relative overflow-hidden shadow-sm ${
              appt.status === "completed" 
                ...
                : "bg-white border-slate-200 hover:border-primary hover:shadow-[0_8px_30px_rgba(37,99,235,0.12)] hover:-translate-y-1"
            }`}
          >
            <div className="flex justify-between items-center">
              <span className="text-slate-900 font-bold">{appt.name}</span>
              <span className="text-primary font-black">{appt.price}</span>
            </div>
            <div className="flex justify-between items-center text-sm text-slate-500 font-medium">
              ...
              <span className="bg-primary/10 text-primary px-3 py-1 rounded-md">{appt.product}</span>
            </div>

            {/* Action overlay on hover for pending */}
            {appt.status === "pending" && (
              <div className="absolute inset-0 bg-primary/10 backdrop-blur-[2px] opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                <div className="bg-primary text-white px-5 py-2 rounded-full text-sm font-bold flex items-center gap-1.5 shadow-lg transform hover:scale-105 transition-transform">
