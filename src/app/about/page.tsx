    ...
    <div className="min-h-screen bg-[#f5f4ef] text-slate-950">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-36 sm:px-8 lg:px-12">
        <section className="max-w-4xl">
          <p className="mb-5 text-xs font-black uppercase tracking-[0.18em] text-[#3158e8]">Sobre Bills</p>
        ...
        <section className="mt-20 grid gap-4 sm:grid-cols-2">
          {pillars.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-3xl border border-slate-300 bg-[#fffef9] p-7"><Icon className="h-6 w-6 text-[#3158e8]" aria-hidden="true" /><h2 className="mt-8 text-2xl font-black tracking-[-0.05em]">{title}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{text}</p></article>)}
        </section>
        <section className="mt-20 flex flex-col justify-between gap-8 border-t border-slate-300 pt-8 md:flex-row md:items-end"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#3158e8]">¿Listo para ordenar?</p><h2 className="mt-4 text-4xl font-black tracking-[-0.07em] sm:text-5xl">Empezá con el negocio que ya tenés.</h2></div><Link href="/register" className="inline-flex min-h-12 items-center gap-2 self-start rounded-xl bg-slate-950 px-5 text-sm font-black text-white transition hover:bg-[#3158e8] md:self-auto">Probá Bills gratis <ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link></section>
      </main>
      <Footer />
    </div>
  );
