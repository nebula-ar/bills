...
import { getStaffDaySales } from "@/modules/sales/get-staff-day-sales.use-case";
...
import { formatQuantity, ONE } from "@/lib/quantity";
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <main className="flex min-h-[100dvh] justify-center bg-slate-100 text-slate-950 sm:px-6 sm:py-8">
      <section className="flex h-[100dvh] w-full max-w-md flex-col overflow-hidden bg-white sm:h-[calc(100dvh-4rem)] sm:rounded-[2.5rem] sm:shadow-[0_24px_80px_rgba(15,23,42,0.16)] sm:ring-1 sm:ring-slate-200 lg:max-w-3xl">
        {children}
      </section>
    </main>
  );
...
export default async function MisVentasPage() {
  ...
  const sessionStaff = branch?.users.find((staff) => staff.id === session.staffId) ?? null;
  const staffName = sessionStaff?.name ?? "Vos";
  const saleHref = `/terminal?branch=${session.branchId}&staff=${session.staffId}`;

  const sales = await getStaffDaySales({ branchId: session.branchId, staffId: session.staffId, now: new Date() });
  ...
  return (
    <Shell>
          ...
          <ul className="space-y-2.5 lg:grid lg:grid-cols-2 lg:gap-3 lg:space-y-0">
            {sales.map((sale) => {
              ...
              const itemsLabel = sale.items
                .map((item) => `${item.description}${item.quantity > ONE ? ` ×${formatQuantity(item.quantity)}` : ""}`)
                .join(" · ");
      ...
      <StaffTerminalNav active="sales" saleHref={saleHref} showCashClose={Boolean(sessionStaff?.canCloseCash)} />
    </Shell>
  );
}
