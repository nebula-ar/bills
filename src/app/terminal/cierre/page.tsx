...
export default async function StaffCashClosePage() {
    ...
    <Shell>
      <div className="shrink-0 border-b border-slate-100 px-5 py-4">
        ...
        <p className="mt-0.5 truncate text-sm font-bold text-slate-500">
          {staff?.name} · {branch.name}
        </p>
      </div>

      <form action={submitStaffCashClose} className="flex min-h-0 flex-1 flex-col">
      ...
      <StaffTerminalNav active="cash" saleHref={saleHref} showCashClose />
    </Shell>
  );
}
